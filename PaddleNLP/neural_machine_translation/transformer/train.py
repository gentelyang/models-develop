...
def train(args):
    ...
    if args.local:
        ...
        if args.enable_ce:
            ...
            t.transpile(
                os.getenv("trainer_id"),
                program=train_prog,
                pservers=os.getenv("pserver_endpoints"),
                trainers=os.getenv("num_trainers"),
                sync_mode=args.sync,
                startup_program=startup_prog)
            if training_role == "PSERVER":
                pserver_prog = t.get_pserver_program(os.getenv("current_endpoint"))
                pserver_startup = t.get_startup_program(os.getenv("current_endpoint"),
                                                        pserver_prog)
